...
/**
 * ==========================================================================
 * admin/coupons/edit.php — Modify Existing Coupon
 * ==========================================================================
 */

declare(strict_types=1);

$pageTitle = 'Edit Coupon — GroCo Admin';
...
try {
    ...
    if (!$c) {
        flash('coupon_msg', 'Coupon details not found.', 'error');
        header('Location: index.php');
        exit;
    }
} catch (PDOException $e) {
    error_log('[admin/coupons/edit] load failed: ' . $e->getMessage());
    header('Location: index.php');
    exit;
}
    ...
    if (!verify_csrf()) {
        ...
        if (empty($code) || empty($validFrom) || empty($validUntil)) {
            ...
            try {
                ...
                $check = $pdo->prepare("SELECT COUNT(*) FROM coupons WHERE code = :code AND id != :id");
                $check->execute(['code' => $code, 'id' => $couponId]);
                if ((int)$check->fetchColumn() > 0) {
                    ...
                    $stmt = $pdo->prepare("
                        UPDATE coupons SET
                            ...
                            usage_limit = :limit, valid_from = :from, 
                            valid_until = :until, is_active = :active
                        WHERE id = :id
                    ");
                    $stmt->execute([
                        'code'   => $code,
                    ...
                    log_admin_activity('coupons.edit', "Updated coupon discount code: '{$code}'");
                    flash('coupon_msg', "Coupon Code '{$code}' updated successfully!", 'success');
                    header('Location: index.php');
                    exit;
                }
            } catch (PDOException $e) {
                error_log('[admin/coupons/edit] Save failed: ' . $e->getMessage());
                $error = 'Failed to update coupon record due to database error.';
            }
        }
    }
}
...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:var(--space-5);">
    <div>
        <h1 style="font-size:var(--fs-xl); font-weight:800; color:var(--color-text); margin:0;">Edit Coupon Code</h1>
        <p style="font-size:var(--fs-sm); color:var(--color-text-muted); margin:4px 0 0 0;">Update coupon values and validations.</p>
    </div>
    ...
    <form method="post" class="auth-form">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;" class="grid-2">
            <div class="form-field-group">
                ...
                <input type="text" id="couponCode" name="code" required value="<?= e($c['code']) ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
            <div class="form-field-group">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;" class="grid-2">
            <div class="form-field-group">
                ...
                <input type="number" id="couponPct" name="discount_percent" min="0" max="100" step="0.01" value="<?= (float)$c['discount_percent'] ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
            <div class="form-field-group">
                <label for="couponAmt" style="font-weight:700;">Fixed Discount Amount (৳)</label>
                <input type="number" id="couponAmt" name="discount_amount" min="0" step="0.01" value="<?= (float)$c['discount_amount'] ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;" class="grid-2">
            <div class="form-field-group">
                ...
                <input type="number" id="couponMin" name="min_order_amount" min="0" step="0.01" value="<?= (float)$c['min_order_amount'] ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
            <div class="form-field-group">
                <label for="couponMax" style="font-weight:700;">Maximum Discount (৳)</label>
                <input type="number" id="couponMax" name="max_discount_amount" min="0" step="0.01" value="<?= (float)$c['max_discount_amount'] ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;" class="grid-2">
            <div class="form-field-group">
                ...
                <input type="date" id="couponFrom" name="valid_from" required value="<?= date('Y-m-d', strtotime($c['valid_from'])) ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
            <div class="form-field-group">
                <label for="couponUntil" style="font-weight:700;">Valid Until Date *</label>
                <input type="date" id="couponUntil" name="valid_until" required value="<?= date('Y-m-d', strtotime($c['valid_until'])) ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;" class="grid-2">
            <div class="form-field-group">
                ...
                <input type="number" id="couponLimit" name="usage_limit" min="1" value="<?= (int)$c['usage_limit'] ?>" style="width:100%; padding:8px 12px; border:1px solid var(--color-border); border-radius:var(--radius-sm); font-size:var(--fs-sm); outline:none;">
            </div>
            <div class="form-field-group">
